      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden font-['DM_Sans']">
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold text-slate-900">Start Your Audit</h2>
              <p className="text-slate-500 text-sm mt-1">Add URLs or upload screenshots for AI-powered UX analysis</p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">

            {/* 1. THE OMNI-INPUT BAR */}
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-between sm:items-center gap-4 pt-4 mt-2">
              ...
              <button
                type="submit"
                disabled={isLoading || (queue.length === 0 && !currentUrl.trim())}
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {/* Icon Box */}
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center shadow-sm transition-colors ${(queue.length === 0 && !currentUrl.trim())
                        ? 'bg-slate-300 text-slate-500' // Disabled State Colors
                        : 'bg-white text-indigo-600'    // Active State Colors
                      }`}>
